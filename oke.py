...
logo = """
  __  __ ____  _____
 |  \/  | __ )|  ___| *au : Nisar Mahsud
 | |\/| |  _ \| |_    *fb : fb.com/MAHUDNISAR3
 | |  | | |_) |  _|   *gh : github.com/Mahusdnisar3
 |_|  |_|____/|_|     *yt : youtube.com/Nisar Ahmad
"""
ips=None
try:
...
def login():
    os.system('clear')
    print logo
    print "\n *! Type *T* If Login Using Token"
    print " *! Type *C* If Login Using Cookies"
    lg = raw_input('\n *-> Input : ')
    if lg == '':
        os.sys.exit()
    elif lg == 'T' or lg == 't':
...
def menu():
  ...
  print (' *4 Crack ID From Like')
  print (' *5 View Crack Results')
  print (' *0 Exit (Clear Token/Cookies)\n')
  mn=raw_input(" *-> Input : ")
  if mn=="":
	print ('*! Fill in Correct')
	menu()
  elif mn=="1":
    teman()
  elif mn=="2":
...
def teman():
		...
		limit = '5000'
			...
			raw_input(" *Return")
                ...
                id = []
                z=json.loads(r.text)
                qq = ('friend.txt').replace(" ","_")
                ys = open(qq , 'w')#.replace(" ","_")
                for a in z['data']:
                        id.append(a['id']+"<=>"+a['name'])
                        ys.write(a['id']+"<=>"+a['name']+'\n')
                        print("\r *-> Collecting %s ID\r"%(str(len(id)))),;sys.stdout.flush();time.sleep(0.007)
                ys.close()
                os.rename(qq,file)
		print " "
...
def publik():
	...
	try:
		...
		for a in z['friends']['data']:
			id.append(a['id']+"<=>"+a['name'])
			ys.write(a['id']+"<=>"+a['name']+'\n')
			print("\r *-> Collecting %s ID"%(str(len(id)))),;sys.stdout.flush();time.sleep(0.007)
		ys.close()
		os.rename(qq,file)
		print("\r *-> Total ID : %s          "%(len(id)))
		metode()
	...
	def __init__(self):
		...
		while True:
			...
			if f=="":continue
				...
				print(' *--> Cp Results Stored In Cp.txt\n')
